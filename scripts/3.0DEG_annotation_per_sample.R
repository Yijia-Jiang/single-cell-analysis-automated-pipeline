...
saveRDS(markers, file.path(analysis_out,paste0(sample,"_DEG_markers_by_cluster.rds")))
write.csv(markers, file.path(analysis_out,paste0(sample,"_DEG_markers_by_cluster.csv")))

print("Perform Annotation using Breast markers")
...
head(RNA@meta.data)
pdf(file.path( analysis_out, paste0(sample, "_UMAP_by_celltype_breast_markerbased.pdf")))
p1 <- DimPlot(RNA, reduction = "umap",  group.by = "assign.ident", label=TRUE) + ggtitle("scRNA Annotation")
print(p1)
dev.off()

png(file.path( analysis_out, paste0(sample, "_UMAP_by_celltype_breast_markerbased.png")),  width=10, height=12, units='in', res=300)
p1 <- DimPlot(RNA, reduction = "umap",  group.by = "assign.ident", label=TRUE) + ggtitle("scRNA Annotation")
print(p1)
dev.off()
...
png(file.path(analysis_out, paste0(sample, "_UMAP_feature_plot_breast_markers.png")), width=16, height=4*ceiling(length(markers2)/5.0), units='in', res=300)
...
png(file.path(analysis_out, paste0(sample,"_UMAP_feature_plot_immune_markers.png")), width=16, height=4*ceiling(length(markers2)/3.0), units='in', res=300)
...
head(RNA@meta.data)
pdf(file.path( analysis_out, paste0(sample, "_UMAP_by_celltype_cibersort_markerbased.pdf")))
p1 <- DimPlot(RNA, reduction = "umap",  group.by = "assign.ident.cibersort", label=TRUE) + ggtitle("scRNA Annotation")
print(p1)
dev.off()

png(file.path( analysis_out, paste0(sample, "_UMAP_by_celltype_cibersort_markerbased.png")),  width=10, height=12, units='in', res=300)
p1 <- DimPlot(RNA, reduction = "umap",  group.by = "assign.ident.cibersort", label=TRUE) + ggtitle("scRNA Annotation")
print(p1)
dev.off()
