...
r_inputFiles = strsplit(args[7],' ')[[1]]
...
if(length(r_inputFiles) == 1) {
  print("There is only one ATAC object; no need to merge")
  data <- r_inputFiles[1]
  atacAggr <- readRDS(data)
  DefaultAssay(atacAggr) <- "peaks"
  atacAggr$dataset <- samplenames[1]
  ...
  pdf(umap1_dir)
  p1 <- DimPlot(atacAggr, group.by = "orig.ident")
  print(p1)
  dev.off()
  
  saveRDS(atacAggr, file.path(output_data_dir))
...
saveRDS(combined.peaks, file.path(plot_folder, paste0(integrate_sample_name, "_combined_peaks.rds")))
...
saveRDS(count_list,  file.path(plot_folder, paste0(integrate_sample_name, "_concat_count_lists.rds")))
...
# pdf("v3_analysis_240409/UMAP_before_integration.pdf")
pdf(umap1_dir)
