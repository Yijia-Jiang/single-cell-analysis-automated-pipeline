...
for (i in c(single_sample_names_atac)){
  res2 <- c(
    res2,
    make.tabs(samplename = i, 
              images = c(file.path(single_path_atac , i, paste0(i, '_QC_plot_before_original.png')),
                         file.path(single_path_atac , i, paste0(i, "_QC_plot_after_original.png")),
                         file.path(single_path_atac , i, paste0(i, "_UMAP_by_cluster.png")),
                         file.path(single_path_atac , i, paste0("scATAnno_", atlas), "celltype_assignment/query_assignment/Legend_side", paste0( "umap_3.query_cluster_annotation.png"))
...
for (i in c(merge_sample_names_atac)){
  res2 <- c(
    res2,
    make.tabs(samplename = i,
              images = c(file.path(integrate_path_atac , i, paste0(i, '_UMAP_by_sample_before_integration.png')),
                         file.path(integrate_path_atac , i, paste0(i, "_UMAP_by_sample_after_integration.png")),
                         file.path(integrate_path_atac , i, paste0("scATAnno_", atlas), "celltype_assignment/query_assignment/Legend_side", paste0( "umap_3.query_cluster_annotation.png"))
              ),
...
for (i in c(rna_atac_merge_sample_names)){
  res2 <- c(
    res2,
    make.tabs(samplename = i,
              images = c(file.path(integrate_path_scrna_scatac , i, paste0(i, '_RNA_ATAC_coembed_assay.png')),
                         file.path(integrate_path_scrna_scatac , i, paste0(i, '_RNA_ATAC_coembed_sample.png')),
                         file.path(integrate_path_scrna_scatac , i, paste0(i, "_RNA_ATAC_coembed_celltype.png"))
...
# setwd(plot_folder) 
# Render the Rmd created into html here
# rmarkdown::render(plot_folder , output_file = outputname)
# rmarkdown::render(outputname , output_file = file.path( "summary_report.html"))


codefile <- paste(project_dir, plot_folder, outputname, sep = "/")
outfile <- paste(project_dir, plot_folder, "myreport.html", sep = "/")
