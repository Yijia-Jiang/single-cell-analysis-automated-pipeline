...
sample_path <- args[1] # 
output_dir <- args[2] # 
nCount_RNA_min = args[3] 
nCount_RNA_max = args[4]
mito_rate_max = args[5]
...
# png(file.path(plot_dir, paste0(sample, "_QC_plot_before.png")), width = 3, height = 3, units='in', res=300)
# p <- VlnPlot(seurat, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
# print(p)
# dev.off()
# print("before QC")
...
# png(file.path(plot_dir, paste0(sample, "_QC_plot_before_nowidth.png")))
# p <- grid.arrange(r1,r2,r3,ncol = 4)
# print(p)
# dev.off()
# 
# png(file.path(plot_dir, paste0(sample, "_QC_plot_before_1.png")),width = 1, height = 1, units='in', res=300)
# p <- grid.arrange(r1,r2,r3,ncol = 4)
# print(p)
# dev.off()
# 
# png(file.path(plot_dir, paste0(sample, "_QC_plot_before_2.png")),width = 2, height = 2, units='in', res=300)
# p <- grid.arrange(r1,r2,r3,ncol = 4)
# print(p)
# dev.off()
# 
# png(file.path(plot_dir, paste0(sample, "_QC_plot_before_3.png")),width = 3, height = 3, units='in', res=300)
# p <- grid.arrange(r1,r2,r3,ncol = 4)
# print(p)
# dev.off()
# 
# png(file.path(plot_dir, paste0(sample, "_QC_plot_before_3_nores.png")),width = 3, height = 3)
# p <- grid.arrange(r1,r2,r3,ncol = 4)
# print(p)
# dev.off()

# rnaAggr <- subset(seurat, subset = nCount_RNA > nCount_RNA_min  & nCount_RNA < nCount_RNA_max & percent.mt < 5)
...
png(file.path(plot_dir, paste0(sample, "_QC_plot_after.png")),width = 3, height = 3, units='in', res=300)
p <- grid.arrange(r1,r2,r3,ncol = 4)
print(p)
dev.off()

# png(file.path( plot_dir, paste0(sample, "_QC_plot_after.png")), width = 10, height = 10, units='in', res=300)
# p <- VlnPlot(rnaAggr, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
# print(p)
# dev.off()
...
png(file.path(plot_dir, paste0(sample, "_UMAP_by_cluster_number.png")), width = 3, height = 3, units='in', res=300)
# p1 <- DimPlot(rnaAggr, reduction = "umap",  label = TRUE) + NoLegend()+ ggtitle("scRNA Seurat Clustering")
