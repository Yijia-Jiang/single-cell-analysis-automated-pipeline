...
inputfile_samples <- unlist(lapply(bed_inputFiles, function(s) head(tail(strsplit(s, "/", fixed = TRUE)[[1]], n=2), n=1)))
names(bed_inputFiles) <- inputfile_samples
# 
...
for(i in seq(length(bed_inputFiles))){
  ...
  T1 <- CreateSeuratObject(T1_assay, assay = "ATAC", meta.data = md.T1, project=samplenames[i])
