...
sample_path <- args[1] # sample_path <- "Merge_all/Merge_all_final.rds" "BA-1_209_NormR_Object_Annotated.rds" # BA-1_209_NormR_final.rds
df_cluster_path <- args[2]
df_celltype_path <- args[3] # df_celltype_path <- "Merge_all/Merge_all_assign.ident.csv"
df_singler1_path <- args[4]
...
plot_DEG_markers <- function(rnaAggr, var_col, analysis_out, num_top=10){
  ...
  pdf(file.path(analysis_out, paste0("DEG_heatmap_", var_col, ".pdf")),width=16, height=16)
  p <- DoHeatmap(rnaAggr, features = top10$gene) + NoLegend()
  print(p)
  dev.off()
  
  markers.to.plot <- unique(top10$gene)
  pdf(file.path(analysis_out, paste0("DEG_dotplot_", var_col, ".pdf")),width=14, height=16)
...
markers_cluster <- plot_DEG_markers(rnaAggr, "assign.ident", analysis_out)
